...
from app.services.paper_reset import maybe_reset_paper_baseline
...
async def lifespan(app: FastAPI):
    ...
    app.state.paper_reset_result = await maybe_reset_paper_baseline()
    ...
    try:
        ...
        return {
            ...
            "ready_checks": {
                ...
                "prediction_activated": prediction.get("phase") == "ACTIVADO",
...
async def scanner_latest(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        text(
            "SELECT id::text, started_at, finished_at, symbols_scanned, candidates_found, status, error_message "
...
async def active_signals(
    ...
    result = await db.execute(
        text(
            "SELECT s.id::text, sy.symbol, s.created_at, s.direction, s.state, s.setup_score, "
            "s.risk_score, s.confidence_pct, s.current_price, s.entry_low, s.entry_high, s.stop_loss, "
            "s.tp1, s.tp2, s.tp3, s.expected_move_min_pct, s.expected_move_max_pct, "
            "s.expected_duration_min_minutes, s.expected_duration_max_minutes, s.reason "
            "FROM signals s JOIN symbols sy ON sy.id = s.symbol_id "
            "WHERE s.is_active = TRUE ORDER BY s.setup_score DESC, s.risk_score ASC, s.created_at DESC LIMIT :limit"
        ),
        {"limit": limit},
    )
