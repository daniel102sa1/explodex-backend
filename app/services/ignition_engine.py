...
def build_ignition_signal(scored: dict[str, Any], prediction: dict[str, Any]) -> dict[str, Any]:
    ...
    This is a technical timing index, not a calibrated probability. It is built
    from direction-aligned flow, spot confirmation, volume expansion, OI,
    order-book pressure, short-term momentum and pre-move preparation.

    The fast path is intentionally unable to bypass master direction, hard veto,
    chase protection, invalidation or Risk Guard. Its only purpose is to avoid
    waiting for redundant legacy timing labels after real-time ignition appears.
    """
