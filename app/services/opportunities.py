...
def classify_signal(setup_score: float, risk_score: float) -> dict[str, Any]:
    ...
    if setup_score >= 70 and risk_score <= 50:
        return {
            ...
            "display": "70-79/100 score",
        }
    return {
        "tier": "NO_TRADE",
        "label": "C",
        "action": "DO_NOT_TRADE",
        "display": "below trade threshold",
    }
...
async def calibration_by_score(db: AsyncSession) -> dict[str, Any]:
    """Calculate observed paper-trade win rate by setup-score bucket.

    A percentage is only considered usable after enough closed paper trades.
    This is historical performance, never certainty about the next trade.
    """
...
async def ranked_opportunities(db: AsyncSession, limit: int = 50) -> dict[str, Any]:
    calibration = await calibration_by_score(db)
    calibration_map = {
        item["score_bucket"]: item
        for item in calibration["buckets"]
    }
    ...
    for row in result.mappings().all():
        item = dict(row)
        setup_score = float(item["setup_score"] or 0)
        risk_score = float(item["risk_score"] or 100)
        tier = classify_signal(setup_score, risk_score)
        bucket_name = _bucket(setup_score)
