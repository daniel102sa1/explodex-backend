...
def _targets_for_lane(heart: dict[str, Any], lane_name: str, lane: dict[str, Any]) -> list[tuple[str, float]]:
    ...
    if lane_name == "SWING_PAPER":
        ...
        return [("TARGET1", _f(plan.get("target1"))), ("TARGET2", _f(plan.get("target2"))), ("TARGET3", _f(plan.get("target3")))]
...
def build_stop_survival_plan(*, heart: dict[str, Any], lane_name: str, lane: dict[str, Any], entry: float) -> dict[str, Any]:
    ...
    if (direction == "LONG" and soft_stop >= entry) or (direction == "SHORT" and soft_stop <= entry):
        return {"version": VERSION, "enabled": False, "reason": "invalid_stop_side", "creates_entry": False, "changes_direction": False}

    frame4 = _d(_d(_d(heart.get("higher_timeframe_context")).get("frames")).get("4h"))
    robust4 = max(_f(frame4.get("robust_bar_range_pct")), _f(frame4.get("atr_pct")), 0.35)
    if lane_name == "SWING_PAPER":
        buffer_pct, max_total_stop_pct, confirmation_minutes, min_net_rr = max(0.25, min(1.50, robust4*0.20)), 7.0, 15, 2.6
        hold_hours = max(4.0, min(48.0, _f(lane.get("max_hold_minutes"), 720.0)/60.0))
    elif lane_name in {"AGGRESSIVE_PAPER", "PRE_EVENT_PAPER"}:
        buffer_pct, max_total_stop_pct, confirmation_minutes, min_net_rr = max(0.20, min(1.20, robust4*0.28)), 4.5, 5, 2.8
        hold_hours = max(2.0, min(6.0, _f(lane.get("max_hold_minutes"), 240.0)/60.0))
    else:
    ...
    if es >= 72 and edir == direction and einv > 0:
        if direction == "LONG" and einv < reference: reference, elliott_used = einv, True
        elif direction == "SHORT" and einv > reference: reference, elliott_used = einv, True

    proposed_hard = reference - entry*buffer_pct/100.0 if direction == "LONG" else reference + entry*buffer_pct/100.0
    cap_stop = entry - entry*max_total_stop_pct/100.0 if direction == "LONG" else entry + entry*max_total_stop_pct/100.0
    hard_stop = max(proposed_hard, cap_stop) if direction == "LONG" else min(proposed_hard, cap_stop)
    if (direction == "LONG" and hard_stop >= soft_stop) or (direction == "SHORT" and hard_stop <= soft_stop):
        return {"version": VERSION, "enabled": False, "reason": "no_safe_extra_room", "creates_entry": False, "changes_direction": False}
